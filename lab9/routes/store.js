...
app.get('/edit/(:id)', function(req, res, next){
    ...
    */
    req.getConnection(function(error, conn) {
        conn.query('select * from store where id like ?', req.params.id, function(err, rows, fields) {
// if item not found
            if (rows.length <= 0) {
...
//Validate price
    ...
    if( !errors ) { //No errors were found. Passed Validation!
        ...
        req.getConnection(function(error, conn) {
            conn.query('UPDATE store SET ? where id like ?', [item, req.params.id], function(err, result) {
                if (err) {
                    req.flash('error', err)
...
app.delete('/delete/(:id)', function(req, res, next) {
    req.getConnection(function(error, conn) {
        conn.query('DELETE FROM store where id like ?',req.params.id, function(err, result) {
            if (err) {
                req.flash('error', err)
// redirect to users list page
                res.redirect('/store')
            } else {
                req.flash('success', 'ID deleted. ID: '+req.params.id)
// redirect to users list page
                res.redirect('/store')
            }
        })
